...
if( ! class_exists( 'aviaElementManager' ) )
{
	...
	{
		const VERSION			= '1.0';				//	Main version - needed to check for main global updates
		...
		public function __construct() 
		{
			...
			if( is_admin() )
			{
				...
				add_action( 'delete_post', array( $this, 'handler_wp_delete_post' ), 10, 1 );
		...
		public function get_elements_state( $source = 'post', $post_id = 0, $initialise = '' )
		{
			...
			$checked = $this->get_checked_elements();
			$states = array();
		...
		public function handler_wp_get_header( $template = null )
		{
			...
			add_action( 'ava_current_post_element_info_available', $this );
		}
		...
		protected function exec_version_update()
		{
			...
			$query_args = array(
						'post_type'		=> Avia_Builder()->get_supported_post_types(),
		 ...
		 * @param int $post_id
		 */
		public function handler_wp_delete_post( $post_id )
		{
			$elements = $this->registered_elements();
		...
		public function debug_element_usage_info()
		{
			...
			{
				$out .=				"[av_toggle title='{$element}' tags='{$element}']";
